...
df = pd.read_csv("../../cadmium_gis/Ottawa_DAs/DA Ottawa Clean.csv")  # General information (id, population, area...)
df_adj = pd.read_csv("../../cadmium_gis/Ottawa_DAs/DA Ottawa Adjacency.csv")  # Pair of adjacent territories
gdf_ontario = gpd.read_file("../../cadmium_gis/Ottawa_DAs/DA Ottawa.gpkg")  # GeoDataFrame with the territories poligons
...
default_cell = json.loads(open("input_ottawa_da/default.json", "r").read())
fields = json.loads(open("input_ottawa_da/fields.json", "r").read())
infectedCell = json.loads(open("input_ottawa_da/infectedCell.json", "r").read())
...
with open("output/scenario_ottawa_da.json", "w") as f:
